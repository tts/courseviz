...
this_is_done <- character(nrow(suositus_united))
this_is_combined_key <- character(nrow(suositus_united))
...
for(i in seq_along(course_codes_from_to_plot_df[[1]])){
  if (!course_codes_from_to_plot_df[[1]][i] %in% c("P", "Zero")) { # skip P and Zero rows
    this_is_done[grepl(x = suositus_united$combined, pattern = course_codes_from_to_plot_df[[1]][i])] <- "done"
    this_is_combined_key[grepl(x = suositus_united$combined, pattern = course_codes_from_to_plot_df[[1]][i])] <- course_codes_from_to_plot_df[[1]][i]
  }
} 
suositus_united_done <- cbind(suositus_united, this_is_done, this_is_combined_key, stringsAsFactors = FALSE)

# If output_vector=='', change Vari to white, i.e. not done yet
courses_done <- suositus_united_done %>% 
  rename(oldVari = Vari) %>% 
...
to_plot_done <- to_plot %>% 
  ungroup() %>% 
  left_join(suositus_united_done, 
