...
course_codes_from_to_plot_df <- list(to_plot$Kurssin.koodi)
...
#
# From that row in recomm_united_done, where there is a matching Kurssin.koodi in this_is_combined_key, 
# take the combined key

to_plot_done <- to_plot %>% 
  ungroup() %>% 
  left_join(recomm_united_done, 
            by= c("Kurssin.koodi"="this_is_combined_key")) %>% 
  mutate(uid = ifelse(is.na(combined), Kurssin.koodi, combined)) %>% # if the course is not done yet, use the course code 
  mutate(Lukukausi = Lukukausi.x) %>% 
  mutate(Vuosi = Vuosi.x) %>% 
  mutate(Opintopisteet = Opintopisteet.x) %>% 
  mutate(Vari = Vari.x) %>% 
  select(Opiskelijanumero, Nimi, Kurssin.koodi, Kurssin.nimi, Opintopisteet, Lukukausi, Vuosi, Vari, schoolyear, schoolyearrange,
