...
draw <- function(df) {
  ...
  p <- ggplot(df, aes(ymin = -0.20)) + 
    ...
    geom_text(aes(x = w-term_label_x_offset, # Opintopisteet per lukukausi
                  ...
                  label = termsum),
              size = font_size,
              colour=ifelse(df$Lukukausi == "Syksy", colour_term_autumn_sum_points, colour_term_spring_sum_points)) +
    geom_text(aes(x = wt, # Opintopisteet
                  y = height + 0.1,
                  label = ifelse(width != width_absent, width, "")),
              #   ifelse(Kurssin.koodi == 'P' || Kurssin.koodi == 'Zero', "", 0))),
              size = font_size) +
    geom_text(aes(x = wt, # Kurssikoodi ja nimi
                  y = height * 0.5,
                  label = ifelse(Kurssin.koodi != "P" & height > 2 , paste0(Kurssin.koodi, " ",Kurssin.nimi), 
                                 ifelse(Kurssin.koodi != "P" & height == 2, Kurssin.koodi,
                                        ifelse(Kurssin.koodi != "P" & height <= 1, "", 
                                               ""))),
                  angle = 90),
              size = font_size,
...
filterdata <- function(df, p) {
  ...
  zero_terms_df <- zero_terms %>% 
    ...
    mutate(Kurssin.koodi = "Zero") %>% 
    mutate(Kurssin.nimi = "Ei suorituksia, ei poissaoloa") %>% 
    ...
    mutate(tooltip = paste0(schoolyearrange, " ", 
                            ifelse(Lukukausi=="Syksy", "Syksy", "Kevat"), # hack 
                            ": Ei suorituksia")) %>% 
    select(Opiskelijanumero,Nimi,Kurssin.koodi,Kurssin.nimi,Opintopisteet,Lukukausi,Vuosi,Vari,schoolyear,schoolyearrange,color,height,width,tooltip)
...
data$Kurssin.nimi <- gsub("'", "", data$Kurssin.nimi)

# Add color
data_joined <- left_join(data, colordata, by=c("Kurssin.koodi"= "Kurssikoodi"))
...
coursedata <- data_joined %>%
  ...
  mutate(color = ifelse(is.na(Vari) & Kurssin.koodi != 'P', colour_course_default, 
                        ifelse(is.na(Vari) & Kurssin.koodi == 'P', colour_absent, 
                               cColors$color[match(Kurssin.koodi, cColors$course)]))) %>% 
  ...
  rename(width = width2) %>%
  mutate(tooltip = ifelse(Kurssin.nimi != '',
                          paste0(schoolyearrange, " ", 
                                 ifelse(Lukukausi=="Syksy", "Syksy", "Kevat"), # hack
                                 ": ", Kurssin.nimi, " (", Arvosana, ")"),
                          paste0(schoolyearrange, " ", 
                                 ifelse(Lukukausi=="Syksy", "Syksy", "Kevat"), # hack
                                 ": Poissaolo"))) %>%
...
sapply(unique(coursedata$Nimi), function(x) {
  ...
  sapply("html", function(y) {
    ...
    rmarkdown::render(paste0("template_", y, "_win.Rmd"),
